...
public class InventoryManager : MonoBehaviour
{
    ...
    public bool CanConsume(IReadOnlyList<Item_Amount> costs)
    {
        ...
        {
            Item_Amount cost = costs[i];
            if (cost == null || cost.itemData == null)
            {
                continue;
            }

            if (!HasItem(cost.itemData, cost.SafeQuantity))
            {
                return false;
            }
        }
    ...
    public bool TryConsume(IReadOnlyList<Item_Amount> costs)
    {
        if (!CanConsume(costs))
        {
            return false;
        ...
        for (int i = 0; i < costs.Count; i++)
        {
            ...
            RemoveItem(cost.itemData.itemID, out _, cost.SafeQuantity);
        }

        return true;
    }

    public void Grant(IReadOnlyList<Item_Amount> rewards)
    {
        if (rewards == null)
        {
            return;
        ...
        {
            Item_Amount reward = rewards[i];
            if (reward == null || reward.itemData == null)
            {
                continue;
            }

            AddItem(reward.itemData, reward.SafeQuantity);
        }
    }
